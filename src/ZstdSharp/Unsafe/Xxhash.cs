...
namespace ZstdSharp.Unsafe
{
    public static unsafe partial class Methods
    {
        ...
        private static uint XXH_read32(void* memPtr)
        {
            ...
            XXH_memcpy((void*)&val, memPtr, (nuint)(sizeof(uint)));
            return val;
        }
        ...
        private static uint XXH32_finalize(uint h32, byte* ptr, nuint len, XXH_alignment align)
        {
...

            {
                ...
                while (len >= 4)
                {
                    ...
                    {
                        h32 += XXH_readLE32_align((void*)ptr, align) * 0xC2B2AE3DU;
                        ptr += 4;
        ...
        private static uint XXH32_endian_align(byte* input, nuint len, uint seed, XXH_alignment align)
        {
            ...
            if (len >= 16)
            {
                ...
                do
                {
                    v1 = XXH32_round(v1, XXH_readLE32_align((void*)input, align));
                    input += 4;
                    v2 = XXH32_round(v2, XXH_readLE32_align((void*)input, align));
                    input += 4;
                    v3 = XXH32_round(v3, XXH_readLE32_align((void*)input, align));
                    input += 4;
                    v4 = XXH32_round(v4, XXH_readLE32_align((void*)input, align));
                    input += 4;
                }
                while (input < limit);
        ...
        {
            XXH_memcpy((void*)dstState, (void*)srcState, (nuint)(sizeof(XXH32_state_s)));
        }

        public static XXH_errorcode ZSTD_XXH32_reset(XXH32_state_s* statePtr, uint seed)
        {
            memset((void*)statePtr, 0, (nuint)(sizeof(XXH32_state_s)));
        ...
        public static XXH_errorcode ZSTD_XXH32_update(XXH32_state_s* state, void* input, nuint len)
        {
...

            {
                ...
                state->large_len |= (uint)((((len >= 16) || (state->total_len_32 >= 16))) ? 1U : 0U);
                ...
                if (state->memsize != 0)
                {
                    XXH_memcpy((void*)((byte*)(state->mem32) + state->memsize), input, 16 - state->memsize);

                    {
                        uint* p32 = (uint*)state->mem32;

                        state->v[0] = XXH32_round(state->v[0], XXH_readLE32((void*)p32));
                        p32++;
                        state->v[1] = XXH32_round(state->v[1], XXH_readLE32((void*)p32));
                        p32++;
                        state->v[2] = XXH32_round(state->v[2], XXH_readLE32((void*)p32));
                        p32++;
                        state->v[3] = XXH32_round(state->v[3], XXH_readLE32((void*)p32));
                    }
                ...
                if (p <= bEnd - 16)
                {
                    ...
                    do
                    {
                        state->v[0] = XXH32_round(state->v[0], XXH_readLE32((void*)p));
                        p += 4;
                        state->v[1] = XXH32_round(state->v[1], XXH_readLE32((void*)p));
                        p += 4;
                        state->v[2] = XXH32_round(state->v[2], XXH_readLE32((void*)p));
                        p += 4;
                        state->v[3] = XXH32_round(state->v[3], XXH_readLE32((void*)p));
                        p += 4;
                    }
                    while (p <= limit);
                ...
                if (p < bEnd)
                {
                    XXH_memcpy((void*)state->mem32, (void*)p, (nuint)(bEnd - p));
        ...
        public static void ZSTD_XXH32_canonicalFromHash(XXH32_canonical_t* dst, uint hash)
        {
            ...
            XXH_memcpy((void*)dst, (void*)&hash, (nuint)(sizeof(XXH32_canonical_t)));
        }

        public static uint ZSTD_XXH32_hashFromCanonical(XXH32_canonical_t* src)
        {
            return XXH_readBE32((void*)src);
        }
        ...
        private static ulong XXH_read64(void* memPtr)
        {
            ...
            XXH_memcpy((void*)&val, memPtr, (nuint)(sizeof(ulong)));
            return val;
        }
        ...
        private static ulong XXH64_finalize(ulong h64, byte* ptr, nuint len, XXH_alignment align)
        {
            ...
            while (len >= 8)
            {
                ulong k1 = XXH64_round(0, XXH_readLE64_align((void*)ptr, align));
            ...
            if (len >= 4)
            {
                h64 ^= (ulong)(XXH_readLE32_align((void*)ptr, align)) * 0x9E3779B185EBCA87UL;
                ptr += 4;
        ...
        private static ulong XXH64_endian_align(byte* input, nuint len, ulong seed, XXH_alignment align)
        {
            ...
            if (len >= 32)
            {
                ...
                do
                {
                    v1 = XXH64_round(v1, XXH_readLE64_align((void*)input, align));
                    input += 8;
                    v2 = XXH64_round(v2, XXH_readLE64_align((void*)input, align));
                    input += 8;
                    v3 = XXH64_round(v3, XXH_readLE64_align((void*)input, align));
                    input += 8;
                    v4 = XXH64_round(v4, XXH_readLE64_align((void*)input, align));
                    input += 8;
                }
                while (input < limit);
        ...
        {
            XXH_memcpy((void*)dstState, (void*)srcState, (nuint)(sizeof(XXH64_state_s)));
        }

        public static XXH_errorcode ZSTD_XXH64_reset(XXH64_state_s* statePtr, ulong seed)
        {
            memset((void*)statePtr, 0, (nuint)(sizeof(XXH64_state_s)));
        ...
        public static XXH_errorcode ZSTD_XXH64_update(XXH64_state_s* state, void* input, nuint len)
        {
...

            {
                ...
                if (state->memsize != 0)
                {
                    XXH_memcpy((void*)(((byte*)(state->mem64)) + state->memsize), input, 32 - state->memsize);
                    state->v[0] = XXH64_round(state->v[0], XXH_readLE64((void*)(state->mem64 + 0)));
                    state->v[1] = XXH64_round(state->v[1], XXH_readLE64((void*)(state->mem64 + 1)));
                    state->v[2] = XXH64_round(state->v[2], XXH_readLE64((void*)(state->mem64 + 2)));
                    state->v[3] = XXH64_round(state->v[3], XXH_readLE64((void*)(state->mem64 + 3)));
                    p += 32 - state->memsize;
                    state->memsize = 0;
                }
                ...
                if (p + 32 <= bEnd)
                {
                    ...
                    do
                    {
                        state->v[0] = XXH64_round(state->v[0], XXH_readLE64((void*)p));
                        p += 8;
                        state->v[1] = XXH64_round(state->v[1], XXH_readLE64((void*)p));
                        p += 8;
                        state->v[2] = XXH64_round(state->v[2], XXH_readLE64((void*)p));
                        p += 8;
                        state->v[3] = XXH64_round(state->v[3], XXH_readLE64((void*)p));
                        p += 8;
                    }
                    while (p <= limit);
                ...
                if (p < bEnd)
                {
                    XXH_memcpy((void*)state->mem64, (void*)p, (nuint)(bEnd - p));
                    state->memsize = (uint)(bEnd - p);
                }
            }
        ...
        public static void ZSTD_XXH64_canonicalFromHash(XXH64_canonical_t* dst, ulong hash)
        {
            ...
            XXH_memcpy((void*)dst, (void*)&hash, (nuint)(sizeof(XXH64_canonical_t)));
        }

        public static ulong ZSTD_XXH64_hashFromCanonical(XXH64_canonical_t* src)
        {
            return XXH_readBE64((void*)src);
        }
    }
}
